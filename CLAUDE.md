...
- **Ports `5001`/`3001`**, chosen so a running `./dev.sh` (5000/3000) does not collide. The harness refuses to start if either is taken: Bun sets `SO_REUSEPORT` on Linux, so a leftover server does **not** cause a bind error — it binds the same port and requests round-robin between the two, which fails tests for reasons that look nothing like the cause.
- **Both servers run inside the test process** (`buildApp().listen()`, not a spawned `bun run src/index.ts`). ⚠️ Do not "fix" this back into a child process: `process.on('exit')` does not fire under `bun test`, so a spawned API survives the run and then collides with the next one per the point above.
...
- **All runtime config lives in per-app `.env`** (`apps/{api,web}/.env`), gitignored and auto-loaded by Bun from each app's cwd. **Never hardcode config in source** (no default URLs/ports/secrets) — read from `process.env` and validate with the `required()` helper in `apps/api/src/env.ts`. Committed `.env.example` files list the required vars.
  - `apps/api`: `DATABASE_URL`, `JWT_SECRET`, `CORS_ORIGIN` (+ optional `PORT`, `COOKIE_SECURE`, `COOKIE_DOMAIN`). **In production `CORS_ORIGIN` and `COOKIE_SECURE` are mandatory**: `readEnv` (`src/env.ts`) refuses to boot without them, because a missing `CORS_ORIGIN` silently falls back to `*` and a missing `COOKIE_SECURE` silently drops the `Secure` flag from the session cookies. The trigger is `NODE_ENV=production`, set by the `start` script itself rather than by the deploy config — a mode flag that can be forgotten disables its own check.
